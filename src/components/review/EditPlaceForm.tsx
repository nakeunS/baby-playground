...
export default function EditPlaceForm({ id, place }: EditPlaceFormProps) {
  ...
  const [placeData, setPlaceData] = useState<SelectedPlaceData | null>({
    ...
    photoUrl: place.image_url || '',
  })

  // 기존 이미지 파싱
  const initialImages = place.image_urls 
    ? place.image_urls 
    : place.image_url 
      ? place.image_url.split(',').map((url: string) => url.trim()).filter(Boolean)
      : []
